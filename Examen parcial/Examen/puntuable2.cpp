...
template <class T>
void superUnionDiferencial(const set< set<T> >& S, set<T>& result) {
  result.clear();
  typename set< set<T> >::iterator setIt, setIt2;
  typename set<T>::iterator it;
  bool incluir;
...
int main(){
  ...
  for (int i=0;i<10;i++) {
    ...
    s4.insert(generaEntero(1,20));
    s5.insert(generaEntero(1,20));
  }
  ...
  cout << "Conjunto D = " << s4 << endl << endl;
  cout << "Conjunto E = " << s5 << endl << endl;

  S.insert(s4);
  S.insert(s5);
  superUnionDiferencial(S, result);
  cout << "Super unión diferencial de los cinco = " << result << endl << endl;
